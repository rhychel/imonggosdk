...
public class WH_Finalize extends ModuleActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Log.e("WH_Finalize >>> CUSTOMER", customer.getName());
        ...
        btn1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (((Double) tvBalance.getTag()) < 0) {
                    DialogTools.showDialog(WH_Finalize.this, "Oopss!", "Total return amount cannot be greater than your total sales amount.", R.style.AppCompatDialogStyle_Light);
                    return;
                }
                if (((Double) tvBalance.getTag()) == 0 && ProductsAdapterHelper.getSelectedReturnProductItems().size() > 0) {
                    DialogTools.showDialog(WH_Finalize.this, "Oopss!", "Total amount cannot be zero.", R.style.AppCompatDialogStyle_Light);
                    return;
                }

                /*Intent intent = new Intent(WH_Finalize.this, C_Checkout.class);
                intent.putExtra(REFERENCE, reference);
                intent.putExtra(IS_LAYAWAY, isLayaway);
                startActivityForResult(intent, SALES);*/


                AlertDialog.Builder dialog = new AlertDialog.Builder(WH_Finalize.this, R.style.AppCompatDialogStyle_Light);
                ...
                dialog.setPositiveButton("YES", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        try {
                            ...
                            if (isReceiving && !isDispatching)
                                document = DocumentTools.generateDocument(WH_Finalize.this, getSession().getDevice_id(),
                                        DocumentTypeCode.RECEIVE_SUPPLIER);
                            else if (!isReceiving && isDispatching)
                                document = DocumentTools.generateDocument(WH_Finalize.this, getSession().getDevice_id(),
                                        selectedBranch.getId(),
                                        DocumentTypeCode.RELEASE_BRANCH);
                            Branch warehouseBranch = Branch.fetchById(getHelper(),Branch.class, SettingTools.defaultWarehouse(WH_Finalize.this));
                            Log.e("~~~~~~~~~~~ BRANCH", warehouseBranch.getName() + " ~ " + selectedBranch.getName());
                            if(document != null) {
        ...
        switch (item.getItemId()) {
            case R.id.mReturn: {
                ...
                addAndReturnDialog.setPositiveButton("YES", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        // Go to returning
                        Intent intent = new Intent(WH_Finalize.this, WH_Module.class);
